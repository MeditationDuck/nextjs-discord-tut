...
export default async function handler(
  ...
  try{
    ...
    if(!conversationId) return res.status(400).json({ error: "conversationId missing" });
    ...
    const isModify = isMessageOwner || isAdmin || isModerator;

    if(!isModify) return res.status(401).json({ error: "Unauthorized" });
